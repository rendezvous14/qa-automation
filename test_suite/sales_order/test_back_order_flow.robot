*** Settings ***
Documentation     Verify Back ordered for Sales Order
...
*** Test Cases ***
BACK_ORDERED_01
    [Documentation]  Verify the Sales Order Status is BACK_ORDERED when:
    ...      - 3 Product Items but 1 product is back ordered
    [Tags]   Sales order
    ...      Back ordered
    ...
    Log   No Purchase Order for this items
    ###  ${partnerPurchaseItemId}=   Prepare Purchase Order TH   ${purchase_order_template}  ${partner_id}  ${STATUS_201}  ${partnerItemId}   10
    ...
    ${productLineItems_General}=  Create Product Line Items  ${productLineItem_template}  ${partner_id}  ${partnerItemId_General}   1   2  1  99.99
    ${productLineItems_CoolRoom}=  Create Product Line Items  ${productLineItem_template}  ${partner_id}  ${partnerItemId_CoolRoom}   2  2  1  99.99
    ${productLineItems_HighValue}=  Create Product Line Items  ${productLineItem_template}  ${partner_id}  ${partnerItemId_HighValue}  3  2  1  99.99
    ...
    Verify Sales Order on DB    ${soKey}   orderStatus      BACK_ORDERED
    # Verify Sales Order on DB    ${soKey}   shipmentStatus   PACKED
    Verify Sales Order on DB    ${soKey}   backorder        true
    Verify Sales Order on DB    ${soKey}   onCancel         false
    Verify Sales Order on DB    ${soKey}   deleted          false
    Verify Sales Order on DB    ${soKey}   products         ${partnerItemId_General}    1
    Verify Sales Order on DB    ${soKey}   products         ${partnerItemId_CoolRoom}   2
    Verify Sales Order on DB    ${soKey}   products         ${partnerItemId_HighValue}  3
    ...
    Verify History of Sales Order on DB    ${soKey}       orderStatus     BACK_ORDERED    3
    Verify History of Sales Order on DB    ${soKey}       backorder       false           1
    Verify History of Sales Order on DB    ${soKey}       backorder       false           2
    Verify History of Sales Order on DB    ${soKey}       backorder       true            3

    comment  -------- Back Order Resolved --------
    ### High Value
    ${partnerPurchaseItemId}=   Prepare Purchase Order TH   ${purchase_order_template}  ${partner_id}  ${STATUS_201}  ${partnerItemId}   10    
    Wait until Fulfillment Order Status is PACKED
    comment  -------- Verify data on Order Store DB --------
    ...
    Verify History of Sales Order on DB    ${soKey}       shipmentStatus  RESERVED        5
    Verify History of Sales Order on DB    ${soKey}       orderStatus     IN_PROGRESS     6
    Verify History of Sales Order on DB    ${soKey}       shipmentStatus  WAREHOUSE_SUBMITTED     6
    Verify History of Sales Order on DB    ${soKey}       orderStatus     IN_PROGRESS     7
    Verify History of Sales Order on DB    ${soKey}       shipmentStatus  READY_TO_PICK   7
    Verify History of Sales Order on DB    ${soKey}       orderStatus     IN_PROGRESS     8
    Verify History of Sales Order on DB    ${soKey}       shipmentStatus  PACKED          8
    Verify History of Sales Order on DB    ${soKey}       backorder       false           1
    Verify History of Sales Order on DB    ${soKey}       backorder       false           2
    Verify History of Sales Order on DB    ${soKey}       backorder       true            3
    ...
    Verify Fulfillment Order on DB    ${soKey}   products     ${partnerItemId_General}    1
    Verify Fulfillment Order on DB    ${soKey}   products     ${partnerItemId_CoolRoom}   2
    Verify Fulfillment Order on DB    ${soKey}   products     ${partnerItemId_HighValue}  3
    ...
    @{orderStatus}=     Set Variable    NEW   NEW             NEW             NEW
    @{detailedStatus}=  Set Variable    NEW   ORDERED         READY_TO_PICK   PACKED
    @{visibility}=      Set Variable    15    15              15              7
    ...
    ${idType}=          Set Variable    ITEM_SALES_ORDER
    ${msgType}=         Set Variable    ORDER_STATUS
    @{orderStatus}=     Set Variable    NEW   IN_PROGRESS   BACK_ORDERED
    @{detailedStatus}=  Set Variable    NEW   IN_PROGRESS   BACK_ORDERED
...
BACK_ORDERED_02
    [Documentation]  Verify the Sales Order Status is BACK_ORDERED when:
    ...      - 3 Product Items and all products are back ordered
    [Tags]   Sales order
    ...      Back ordered
    comment  -------- Defined Variables --------
    ${current_date}  Get Current Date
    ...
    comment  -------- Prepare Line Product item --------
    ${productLineItems_General}=  Create Product Line Items  ${productLineItem_template}  ${partner_id}  ${partnerItemId_General}   1   5  1  99.99
    ${productLineItems_CoolRoom}=  Create Product Line Items  ${productLineItem_template}  ${partner_id}  ${partnerItemId_CoolRoom}   2  5  1  99.99
    ${productLineItems_HighValue}=  Create Product Line Items  ${productLineItem_template}  ${partner_id}  ${partnerItemId_HighValue}  3  5  1  99.99
    ${productLineItemsList}=  Catenate  ${productLineItems_General}  ${COMMA}  ${productLineItems_CoolRoom}   ${COMMA}  ${productLineItems_HighValue}
    comment  -------- Create Sales Order --------
    ${soKey}=  Prepare Sales Order    
    ...           ${sales_order_template}
    ...
    comment  -------- Back Order Resolved --------
    ### General
    ${partnerItemId}=    Set Variable   ${partnerItemId_General}
    ${partnerPurchaseItemId}=   Prepare Purchase Order TH   ${purchase_order_template}  ${partner_id}  ${STATUS_201}  ${partnerItemId}   10
    ### Cool Room
    ${partnerItemId}=    Set Variable   ${partnerItemId_CoolRoom}
    ${partnerPurchaseItemId}=   Prepare Purchase Order TH   ${purchase_order_template}  ${partner_id}  ${STATUS_201}  ${partnerItemId}   10
    ### High Value
    ${partnerItemId}=    Set Variable   ${partnerItemId_HighValue}
    ${partnerPurchaseItemId}=   Prepare Purchase Order TH   ${purchase_order_template}  ${partner_id}  ${STATUS_201}  ${partnerItemId}   10
    ...
    comment  -------- Verify Inventory available and onhand --------
    Verify Inventory Items on IMS   ${partnerItemId_General}    5    5    0    0    5   0   0
    Verify Inventory Items on IMS   ${partnerItemId_CoolRoom}   5    5    0    0    5   0   0
    Verify Inventory Items on IMS   ${partnerItemId_HighValue}  5    5    0    0    5   0   0

BACK_ORDERED_03
    [Documentation]  Verify the Sales Order Status is BACK_ORDERED when:
    ...      - Bundle product items but one item is back ordered
    ...      - Multiple items qty
    ...      - Item service
    ...      - Item discount
    [Tags]   Sales Order
    ...
    comment               expected_qty_general    expected_qty_quanrantine
    comment               expected_qty_return
    Wait Until Keyword Succeeds  3x  2s   Verify Inventory Items on IMS   ${partnerItemId_CoolRoom}   10    10    0    0   10   0   0
    Wait Until Keyword Succeeds  3x  2s   Verify Inventory Items on IMS   ${partnerItemId_HighValue}  10    10    0    0   10   0   0
    comment  -------- Prepare Line Product item --------
    ...
    comment   -------- Verify Inventory available and onhand  --------
    Wait Until Keyword Succeeds  3x  2s   Verify Inventory Items on IMS   ${partnerItemId_General}    7    7    0    0   7   0   0
    Wait Until Keyword Succeeds  3x  2s   Verify Inventory Items on IMS   ${partnerItemId_CoolRoom}   7    7    0    0   7   0   0
    Wait Until Keyword Succeeds  3x  2s   Verify Inventory Items on IMS   ${partnerItemId_HighValue}  7    7    0    0   7   0   0